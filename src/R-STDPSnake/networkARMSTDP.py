...
#         nest.Connect(self.neuron_pre_left[:(len(self.neuron_post)//2)], [self.neuron_post[0]], "all_to_all", p.inh_syn_params)
#         nest.Connect(self.neuron_pre_left[:(len(self.neuron_post)//2)], [self.neuron_post[1]], "all_to_all", p.exc_syn_params)
#         nest.Connect(self.neuron_pre_left[(len(self.neuron_post)//2):], [self.neuron_post[1]], "all_to_all", p.inh_syn_params)
#         nest.Connect(self.neuron_pre_left[(len(self.neuron_post)//2):], [self.neuron_post[0]], "all_to_all", p.exc_syn_params)
    ...
    def simulate(self, dvs_data, reward_conditional, reward_collision, area_left, area_right, test):
        nest.SetStatus(self.conn_l, {"n": reward_conditional*p.reward_factor*0.01 - reward_collision})
        nest.SetStatus(self.conn_r, {"n": -reward_conditional*p.reward_factor*0.01 + reward_collision})
